...
shared_ptr<Node> ParseExpression(It& current, It end, unsigned precedence) {
    //если вектор токенов пустой (то есть ввод пустого условия)
    if (current == end) {
        return shared_ptr<Node>();
    }

    shared_ptr<Node> left;
    //если значение равно скобке
...
shared_ptr<Node> ParseCondition(istream& is) {
    ...
    //пустой токен (пробел в условии)
    if (!top_node) {
        top_node = make_shared<EmptyNode>();
    }
    //незнакомый токен после условия, исключение
